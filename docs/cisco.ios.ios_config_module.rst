    ...
    <table  border=0 cellpadding=0 class="documentation-table">
        <tr>
            ...
            <tr>
                ...
                <td>
                        <div>Specifies the source path to the file that contains the configuration or configuration template to load (till ansible-core 2.18).  The path to the source file can either be the full path on the Ansible control host or a relative path from the playbook or role root directory. This argument is mutually exclusive with <em>lines</em>, <em>parents</em>. The configuration lines in the source file should be similar to how it will appear if present in the running-configuration of the device including the indentation to ensure idempotency and correct diff.</div>
                        <div>Post ansible-core 2.18 templating option have changed, src would not render template files by default. It is advised to use the ansible.builtin.template lookup plugin.</div>
