...
class Bfd_static_globalFacts(object):
    ...
    def populate_facts(self, connection, ansible_facts, data=None):
        ...
        bfd_static_global_parser = Bfd_static_globalTemplate(lines=data.splitlines(), module=self._module)
        objs = list(bfd_static_global_parser.parse().values())

        ansible_facts['ansible_network_resources'].pop('bfd_static_global', None)

        params = utils.remove_empties(
            bfd_static_global_parser.validate_config(self.argument_spec, {"config": objs}, redact=True)
        )
