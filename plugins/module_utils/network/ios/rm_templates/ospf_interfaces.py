...
def _tmplt_ip_ospf_authentication(config_data):
    if "authentication" in config_data:
        if config_data.get('afi') == 'ipv4':
            ...
            command = "ipv6 ospf authentication".format(**config_data['process'])
            ...
            if 'dynamic_cost' in config_data['cost']:
                if 'default' in config_data['cost']['dynamic_cost']:
                    ...
                    if 'percent' in config_data['cost']['dynamic_cost']['hysteresis']:
                        command += ' percent {percent}'.format(**config_data['cost']['dynamic_cost']['hysteresis'])
                    elif 'threshold' in config_data['cost']['dynamic_cost']['hysteresis']:
                        command += ' threshold {threshold}'.format(**config_data['cost']['dynamic_cost']['hysteresis'])
                elif 'weight' in config_data['cost']['dynamic_cost']:
                    command += ' dynamic weight'
                    if 'l2_factor' in config_data['cost']['dynamic_cost']['weight']:
                        command += ' L2-factor {l2_factor}'.format(**config_data['cost']['dynamic_cost']['weight'])
                    elif 'latency' in config_data['cost']['dynamic_cost']['weight']:
                        command += ' latency {latency}'.format(**config_data['cost']['dynamic_cost']['weight'])
                    elif 'oc' in config_data['cost']['dynamic_cost']['weight'] and config_data['cost']['dynamic_cost']['weight']['oc']:
                        command += ' oc cdr'
                    elif 'resources' in config_data['cost']['dynamic_cost']['weight']:
                        command += ' resources {resources}'.format(**config_data['cost']['dynamic_cost']['weight'])
                    elif 'throughput' in config_data['cost']['dynamic_cost']['weight']:
                        command += ' throughput {throughput}'.format(**config_data['cost']['dynamic_cost']['weight'])
    return command
    ...
    if "dead_interval" in config_data:
        if config_data.get('afi') ==    'ipv4':
            ...
            if 'time' in config_data['dead_interval']:
                ...
                command += ' minimal hello-multiplier {minimal}'.format(**config_data['dead_interval'])
        elif config_data.get('afi') == 'ipv6':
...
def _tmplt_ip_ospf_neighbor(config_data):
    ...
    if 'poll_interval' in config_data['neighbor']:
            command += ' poll-interval {poll_interval}'.format(**config_data['neighbor']['poll_interval'])
